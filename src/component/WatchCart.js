import React, { Fragment, useState } from "react";

function WatchCart(props) {
  
  const [watches, setWatch] = useState([
    {
      id: "F653R",
      name: " Oris Watch",
      image:
        "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRQmYAO01EYGK8_4pl0Tw3qOg31YBz1m8bqIw&usqp=CAU",
      price: 18000,
      qty: 2,
    },
    {
      id: "WTR4D",
      name: "APPLLP Watch",
      image:
        "https://w0.peakpx.com/wallpaper/421/374/HD-wallpaper-3d-clock-golden-latest-watch-zash.jpg",
      price: 70000,
      qty: 1,
    },
    {
      id: "BA352",
      name: "Unsplash Watch",
      image:
        "https://images.unsplash.com/photo-1587925358603-c2eea5305bbc?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxzZWFyY2h8Mnx8d3Jpc3R3YXRjaHxlbnwwfHwwfHx8MA%3D%3D&w=1000&q=80",
      price: 11000,
      qty: 4,
    },
    {
      id: "DF4K3",
      name: "Boat Watch",
      image:
        "https://www.boat-lifestyle.com/cdn/shop/products/LunarConnect-FI_Black03_1500x.png?v=1678115037",
      price: 25000,
      qty: 7,
    }
  ]);

 
  const IncreQty = (i) => {
     setWatch((prevState) => {
      return prevState.map((watch, index) => {
        if (watch.id === i) {
          return {
            ...watch,
            qty: watch.qty + 1,
          };
        } else {
          return watch;
        }
      });
    });
  };

  const DecreQty = (watchId) => {
    setWatch((prevState) => {
     return prevState.map((watch, index) => {
       if (watch.id === watchId) {
         return {
           ...watch,
           qty: watch.qty - 1 > 0 ? watch.qty -1 : 1
         };
       } else {
         return watch;
       }
     });
   });
 };

 const HendleDelete=(watchId)=>{
 setWatch((prevState)=>{
  return  prevState.filter((watch)=>watch.id!==watchId);
 })
 }


 const TotalFunc=()=>{
    let total=0;
    for (let watch of watches){
      total += (Number(watch.qty)*Number(watch.price));
    }
    return total.toFixed(2);
 }
  return (
    <Fragment>
      <div className="container mt-3">
        <div className="row">
          <div className="col">
            <p className="h3 text-success p-1">Watch Cart</p>
            <p className="lead">
              Lorem ipsum dolor sit amet consectetur adipisicing elit.
              Consectetur sint veritatis totam fugit id blanditiis maiores
              nostrum, iste cum ut perspiciatis reprehenderit possimus
              voluptatum. Ipsum dolorum reprehenderit nesciunt inventore sequi!
            </p>
          </div>
        </div>
        <div className="row mt-3">
          <div className="col">
            <table className="table table-hover text-center table-striped  table-primary">
              <thead>
                <tr>
                  <th>Name</th>
                  <th>Image</th>
                  <th>Price</th>
                  <th>Qty</th>
                  <th>Total</th>
                  <th>Action</th>
                </tr>
              </thead>
              <tbody>
                {watches.map((watch, index) => {
                  return (
                    <tr key={watch.id}>
                      <td>{watch.name}</td>
                      <td>
                        <img
                          src={watch.image}
                          alt="/"
                          width="70px"
                          height="70px"
                        />
                      </td>
                      <td>{watch.price}</td>
                      <td>
                        <i
                          className="bi bi-plus-circle mx-1"
                          onClick={()=>IncreQty(watch.id)}
                        />
                        {watch.qty}
                        <i
                          className="bi bi-dash-circle mx-1"
                          onClick={()=>DecreQty(watch.id)}
                        />
                      </td>
                      <td>{watch.price * watch.qty}</td>
                      <td>
                        <button
                          className="btn btn-danger btn-sm"
                          onClick={()=>HendleDelete(watch.id)}
                        >
                          <i className="bi bi-trash3" />
                        </button>
                      </td>
                    
                    </tr>
                  );
                })}
                <tr>
                   <td>Garnd Total</td>
                    <td>{TotalFunc()}</td>
                </tr>
              </tbody>
            </table>
          </div>
        </div>
      </div>
    </Fragment>
  );
}

export default WatchCart;
